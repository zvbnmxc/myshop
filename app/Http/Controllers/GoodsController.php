...
class GoodsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($goods_name){
            $data=DB::table('goods')->where('goods_name','like','%'.$goods_name.'%')->paginate(2);
        }else{
            $req['goods_name']='';
            $data=DB::table('goods')->orderBy('goods_id','desc')->paginate(2);
        }
        return view('goods/list',['data'=>$data,'goods_name'=>$goods_name]);
    }
    //添加
    ...
    public function store(Request $request)
    {
        ...
        $path = $request->file('goods_tup')->store('goods');
    ...
    public function shanchu(Request $request)
    {
        ...
        $req = $request->all();
        // dd($req);
        $data=DB::table('goods')->where(['goods_id'=>$req['goods_id']])->delete();
        if(!empty($data)){
            return redirect('goods/list');
        }
    }
    ...
    {
        $req = $request->all();
        $data=DB::table('goods')->where(['goods_id'=>$req['goods_id']])->first();
        // dd($data);
    ...
    public function edit(Request $request)
    {
        ...
        $img=asset('storage'.'/'.$path);
        // dd($data);
        $res=DB::table('goods')->where(['goods_id'=>$req['goods_id']])->update([
